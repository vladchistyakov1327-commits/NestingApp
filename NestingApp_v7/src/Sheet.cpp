...
bool Sheet::canPlace(const Polygon& shape) const {
    ...
    for (const auto& pp : placed) {
        Rect pbb = pp.shape.boundingBox();
        // Расширяем bbox на gap для быстрой проверки
        Rect pbbExpanded = pbb.expanded(gap + 0.1);
        if (!bb.intersects(pbbExpanded)) continue;

        // Проверяем пересечение формы
        if (shape.intersects(pp.shape)) return false;

        // ИСПРАВЛЕНИЕ: проверяем зазор gap между деталями
        // (intersects ловит только касание/наложение, но не нарушение зазора)
        if (gap > GEO_EPS) {
            double dist = shape.distanceTo(pp.shape);
            if (dist < gap - 0.1) return false;  // нарушение зазора
        }
    }
    return true;
}
...
Sheet::Placement Sheet::findBestPlacement(
    ...
    for (double angle : angles) {
        ...
        if (ifr.w > GEO_EPS && ifr.h > GEO_EPS) {
            // Шаг сетки: привязываем к bbox детали, а не к IFR
            // Это гарантирует что сетка достаточно плотная для размещения
            Rect partBB = part.boundingBoxRotated(angle);
            double stepX = std::max(1.0, partBB.w * 0.5);
            double stepY = std::max(1.0, partBB.h * 0.5);
            // Но не слишком грубая и не слишком мелкая
            stepX = std::max(1.0, std::min(stepX, ifr.w / 4.0));
            stepY = std::max(1.0, std::min(stepY, ifr.h / 4.0));
        ...
        for (const auto& pos : candidates) {
            ...
            // Bottom-Left эвристика: минимизируем y + 0.7*x (коэффициент из литературы)
            // Но т.к. лист горизонтальный (3000x1500), приоритет y важнее
