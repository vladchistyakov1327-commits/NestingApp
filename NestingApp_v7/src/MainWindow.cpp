...
void NestingWorker::process() {
    ...
    // Скорость резки: UI хранит мм/мин → переводим в мм/с для buildTechCard
...
void MainWindow::setupMenuBar() {
    ...
    menuBar()->addAction("О программе", this, [this]() {
        QMessageBox::about(this, "О NestingApp",
            ...
            "• Geometry: Polygon, NFP, Minkowski sum<br>"
            "• Sheet: Bottom-Left placement via NFP<br>"
            "• GeneticAlgorithm: PMX, островная модель, адаптивные параметры<br>"
            "• DXFLoader: LINE/LWPOLYLINE/ARC/CIRCLE/SPLINE");
    });
}
...
void MainWindow::setupUI() {
    ...
    gaPop_ = new QLineEdit("60");
    gaGen_ = new QLineEdit("300");
...
void MainWindow::onAddDXF() {
    auto files = QFileDialog::getOpenFileNames(
        this, "Выберите DXF файлы", "", "DXF (*.dxf);;All (*)");
...
void MainWindow::onLoadCSV() {
    ...
    while (!in.atEnd()) {
        ...
        // Формат: name, dxf_path[, count]  или  dxf_path, count
        QString dxfPath;
        int     count       = 1;
...
void MainWindow::loadDXFFiles(const QStringList& files) {
    for (const auto& file : files) {
        ...
        for (const auto& w : res.warnings) {
            logView_->append(QString("⚠ %1: %2")
                .arg(QFileInfo(file).fileName())
                .arg(QString::fromStdString(w)));
            statusBar()->showMessage(QString::fromStdString(w), 4000);
        }
        ...
        for (size_t i = 0; i < res.cutContours.size(); ++i) {
            ...
            // Привязываем марки: если деталь одна — берём все марки;
            // если деталей несколько — берём только марки, центроид которых
            // ближе к этой детали, чем к любой другой.
            if (res.cutContours.size() == 1) {
...
void MainWindow::runNesting(NestingMode mode) {
    ...
    statusLabel_->setText("Запуск раскладки...");
...
void MainWindow::updateTechCard(const NestingResult& r) {
    const TechCard& tc = r.techCard; // данные уже вычислены движком
    ...
    for (size_t si = 0; si < r.sheets.size(); ++si) {
        ...
        // Время для этого листа: пропорционально длине реза
    ...
    // Время из движка (рассчитано с правильной скоростью в мм/с)
...
void MainWindow::drawSheet(const Sheet& s) {
    ...
    for (size_t i = 0; i < s.placed.size(); ++i) {
        ...
        for (const auto& v : pp.shape.verts)
            qpoly << QPointF(v.x, H - v.y);   // инверсия Y для Qt
...
void MainWindow::onExportDXF() {
    ...
    for (size_t si = 0; si < result_.sheets.size(); ++si) {
        ...
        QTextStream out(&f);
        out.setLocale(QLocale::c()); // гарантируем точку как десятичный разделитель
