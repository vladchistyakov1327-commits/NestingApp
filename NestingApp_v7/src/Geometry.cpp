...
std::vector<Polygon> Polygon::computeNFP(const Polygon& fixed, const Polygon& moving) {
    ...
    if (aConvex && bConvex) {
        // Оба выпуклые — точный результат
        Polygon nfp = nfpConvexOrbital(A, B);
        nfp.makeCCW();
        return {nfp};
    }

    // Невыпуклые: сначала пробуем прямой орбитальный (работает для «почти выпуклых»)
    Polygon nfpDirect = nfpConvexOrbital(A, B);
    nfpDirect.makeCCW();

    if (!nfpDirect.empty() && nfpDirect.verts.size() >= 3) {
        // Проверяем: если NFP разумного размера — принимаем
        Rect nfpBB = nfpDirect.boundingBox();
        Rect aBB   = A.boundingBox();
        Rect bBB   = B.boundingBox();
        double expectedW = aBB.w + bBB.w;
        double expectedH = aBB.h + bBB.h;

        // Если NFP не слишком мал — принимаем прямой результат
        if (nfpBB.w > expectedW * 0.3 && nfpBB.h > expectedH * 0.3) {
            std::cerr << "[NFP] Невыпуклые, прямой orbital ОК, вершин: "
                      << nfpDirect.verts.size() << "\n";
            return {nfpDirect};
        }
    }

    // Fallback: выпуклые оболочки (консервативно — запрещает больше позиций, но безопасно)
    std::cerr << "[NFP] ПРЕДУПРЕЖДЕНИЕ: невыпуклые -> fallback convexHull"
              << " (A.verts=" << A.verts.size()
              << " B.verts=" << B.verts.size() << ")\n";
    ...
    if (nfp.empty()) {
        std::cerr << "[NFP] ОШИБКА: convexHull orbital тоже вернул пустой NFP!\n";
    } else {
        std::cerr << "[NFP] convexHull NFP вершин: " << nfp.verts.size() << "\n";
    }
