    ...
    NFPCache& cache) const
{
    ...
    // Кэш трансформированных форм: ключ = (partId, angle)
    // Позволяет не пересчитывать rotatedShape для каждой детали в каждой симуляции
    std::unordered_map<std::string, Polygon> shapeCache;
    auto getShape = [&](const Part& part, double angle) -> const Polygon& {
        std::ostringstream k;
        k << part.id << "_" << std::fixed << std::setprecision(1) << angle;
        auto [it, inserted] = shapeCache.emplace(k.str(), Polygon{});
        if (inserted) it->second = part.transformedShape(angle);
        return it->second;
    };

    for (int i = 0; i < (int)order.size(); ++i) {
        const Part& part = *(*parts_)[order[i]];
        (void)getShape(part, angles[i]); // прогрев кэша формы
        auto pl = sheet.findBestPlacement(part, {angles[i]}, cache);
        if (pl.valid) { sheet.place(part, pl.pos, pl.angle); ++placed; }
    }
...
double GeneticAlgorithm::evaluate(const Chromosome& c, NFPCache& cache) const {
    ...
    if (total == 0) return 0;
    if (placed < total) {
        double r = (double)placed / total;
        return r * 0.3 + util * 0.2;
    }
    return util;
}
    ...
    const Chromosome& p1, const Chromosome& p2, std::mt19937& rng) const
{
    ...
    // Углы должны следовать за деталями, а не за позициями!
    // Строим mapping: partIndex → angle из p1 (для сегмента) и p2 (для остального)
    // p1 даёт (order[i], angles[i]) для i в [s..e]
    // p2 даёт (order[i], angles[i]) для i вне [s..e]
    std::unordered_map<int,double> angleMap; // partIndex → angle
    // Сначала p2 (меньший приоритет)
    for (int i = 0; i < n; ++i)
        angleMap[p2.order[i]] = p2.angles[i];
    // Потом p1 для сегмента [s..e] (перекрывает p2)
    for (int i = s; i <= e; ++i)
        angleMap[p1.order[i]] = p1.angles[i];

    // Назначаем угол каждой детали в child по её partIndex
    for (int i = 0; i < n; ++i)
    ...
    Chromosome& c, int maxIter, NFPCache& cache, std::mt19937& rng) const
{
    ...
    for (int k = 0; k < maxIter; ++k) {
        ...
        // Swap order + angles вместе — они составляют пару (деталь, угол)
        std::swap(cand.order[a],  cand.order[b]);
        std::swap(cand.angles[a], cand.angles[b]); // БАГ был: angle не свапался

        // Иногда также пробуем изменить угол у одной из деталей
...
void GeneticAlgorithm::evolveIsland(Island& island, int generation) {
    ...
    // Обновляем лучшего острова
    if (pop[0].fitness > island.best.fitness + 1e-5) {
        island.best = pop[0];
        island.stagnation = 0;
        localSearch(island.best, 6, cache, rng);
    ...
    // Callback прогресса (только от острова 0, определяем по generation)
    (void)generation; // generation используется внешним кодом
}

// ─── migrate ──────────────────────────────────────────────────────────────────
// Топология миграции: кольцо — каждый остров отправляет лучших следующему
void GeneticAlgorithm::migrate(std::vector<Island>& islands) {
    int ni = (int)islands.size();
    if (ni < 2) return;

    // Сохраняем мигрантов от каждого острова
    std::vector<std::vector<Chromosome>> migrants(ni);
    for (int i = 0; i < ni; ++i) {
        int mc = std::min(migrationCount, (int)islands[i].pop.size());
        for (int j = 0; j < mc; ++j)
            migrants[i].push_back(islands[i].pop[j]); // лучшие уже отсортированы
    }

    // Принимаем мигрантов (заменяем худших)
    for (int i = 0; i < ni; ++i) {
        int src = (i - 1 + ni) % ni; // берём от предыдущего
...
// ─── run — основной цикл с островной моделью ─────────────────────────────────
    ...
    const Sheet& sheetTemplate)
{
    ...
    // Число островов: по числу ядер CPU, от 2 до 4
    int ni = islandCount > 0 ? islandCount
           : std::max(2, std::min(4, (int)std::thread::hardware_concurrency()));

    // Размер популяции на остров
    int islandPop = std::max(10, populationSize / ni);

    // Создаём острова
    std::vector<Island> islands(ni);
    for (auto& isl : islands) {
        isl.pop = initPopulation(islandPop, isl.rng, isl.params);
        for (auto& c : isl.pop) c.fitness = evaluate(c, isl.cache);
        std::sort(isl.pop.begin(), isl.pop.end());
        if (!isl.pop.empty()) isl.best = isl.pop[0];
    }

    Chromosome globalBest;
    // Найти лучшего среди всех островов
    auto updateGlobal = [&]() {
        for (auto& isl : islands)
    ...
    for (int gen = 0; gen < maxGenerations; ++gen) {
        ...
        // Параллельная эволюция островов
        ...
        // Миграция каждые migrationEvery поколений
        if ((gen + 1) % migrationEvery == 0)
            migrate(islands);

        // Обновляем глобального лучшего
    ...
    // Объединяем кеши островов в sharedCache_
    for (auto& isl : islands)
