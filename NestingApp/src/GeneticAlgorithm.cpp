...
// ─── evaluate ────────────────────────────────────────────────────────────────
// Целевая функция по Текст1 (адаптирована для максимизации):
//
//   F = w1 * utilization - w2 * (unplaced/total)^2
//
//   где:
//     utilization  — площадь деталей / рабочая площадь листа
//     unplaced     — кол-во неразмещённых деталей (большой штраф)
//     w1=0.4, w2=0.6 — веса
//
//   Диапазон F:
//     placed=0:       F = -0.6           (очень плохо)
//     placed=k<total: F = 0.4*util - 0.6*(1-k/n)²  [-0.6, ~0.35]
//     placed=total:   F = 0.4*util       [0, 0.4]
//
//   Полное размещение ВСЕГДА даёт F >= 0 > F_частичное при ratio < ~0.75
//   Это сильнее штрафует неполное размещение чем предыдущая версия.
double GeneticAlgorithm::evaluate(const Chromosome& c, NFPCache& cache) const {
    ...
    double unplacedRatio = 1.0 - ratio;

    // Штраф за неразмещённые детали (квадратичный — как в Текст1)
    // w2 большое чтобы гарантировать: полное > частичного
    const double w1 = 0.4;  // вес утилизации
    const double w2 = 0.6;  // вес штрафа за неразмещённых

    double fitness = w1 * util - w2 * (unplacedRatio * unplacedRatio);

    // Нормируем в [0, 1] для совместимости с targetFitness=0.96
    // Минимум: 0 - 0.6 = -0.6, максимум: 0.4
    // Нормировка: (fitness + 0.6) / 1.0 → [0, 1.0]
    return (fitness + 0.6);  // теперь: placed=0 → 0.0, placed=total,util=1 → 1.0
}
...
// ─── PMX crossover ────────────────────────────────────────────────────────────
// O(n) реализация:
//   posP1[v]      — позиция значения v в p1 (lookup O(1) вместо std::find O(n))
//   inSegment[v]  — флаг: значение v уже в скопированном сегменте (O(1))
    ...
    const std::vector<int>& p1, const std::vector<int>& p2, int s, int e)
{
    ...
    // Предвычисляем: posP1[v] = индекс v в p1  — O(n)
    std::vector<int> posP1(n);
    for (int i = 0; i < n; ++i)
        posP1[p1[i]] = i;

    // Копируем сегмент [s,e] из p1, помечаем занятые значения  — O(e-s+1)
    ...
    // Заполняем оставшиеся позиции из p2 с разрешением конфликтов  — O(n) total
    // Каждый элемент цепочки mapping обходится не более одного раза
    for (int i = 0; i < n; ++i) {
        if (i >= s && i <= e) continue;
        int val = p2[i];
        int tries = 0;
        // Следуем по цепочке: val → p2[posP1[val]] → ...
        while (inSegment[val] && tries++ < n)
            val = p2[posP1[val]];
        if (tries <= n)
            child[i] = val;
    }

    // Fallback: заполняем оставшиеся -1 пропущенными значениями  — O(n)
...
void GeneticAlgorithm::evolveIsland(Island& island, int generation) {
    ...
    // ─── Иммиграция (Текст1, раздел 5) ──────────────────────────────────────
    // Каждые 50 поколений добавляем случайных «иммигрантов» для разнообразия
    // Борьба с локальными оптимумами
    if (generation > 0 && generation % 50 == 0) {
        int immigrantCount = std::max(1, (int)pop.size() / 10);
        AdaptiveParams resetParams;
        auto immigrants = initPopulation(immigrantCount, rng, resetParams);
        for (auto& imm : immigrants) {
            imm.fitness = evaluate(imm, cache);
        }
        // Заменяем худших в популяции иммигрантами
        for (int ii = 0; ii < (int)immigrants.size(); ++ii) {
            int worstIdx = (int)pop.size() - 1 - ii;
            if (worstIdx >= 0 && immigrants[ii].fitness > pop[worstIdx].fitness * 0.8) {
                pop[worstIdx] = immigrants[ii];
            }
        }
        std::sort(pop.begin(), pop.end());
    }
