...
std::vector<Polygon> Polygon::computeNFP(const Polygon& fixed, const Polygon& moving) {
    ...
    // ─── Невыпуклые детали: декомпозиция + объединение частичных NFP ─────────
    //
    // Стратегия (улучшенная по сравнению с convexHull):
    //   1. Fan-декомпозиция каждого полигона на треугольники от центроида
    //   2. NFP для каждой пары (triA, triB) — точный, выпуклый
    //   3. Объединение: polar-merge — сортируем вершины всех частичных NFP
    //      по углу от общего центроида и строим "звёздчатую" оболочку.
    //      Это точнее convexHull: не "заполняет" вогнутости внутрь NFP.
    //
    // Почему лучше convexHull:
    //   convexHull(allVerts) — выпуклая оболочка ВСЕХ вершин, слишком большая зона
    //   polar-merge — строит границу по максимальному радиусу по каждому углу,
    //   что соответствует "внешней огибающей" множества NFP.
    //
    // Гарантия безопасности: polar-merge ⊇ union(partialNFPs) ⊇ true NFP,
    // поэтому пересечений не будет. Но зона запрета меньше чем convexHull.

    // Функция: fan-декомпозиция от центроида
    ...
    std::vector<Polygon> partialNFPs;

    for (const auto& pa : partsA) {
        for (const auto& pb : partsB) {
            Polygon paC = pa.toConvexHull(); paC.makeCCW();
            Polygon pbC = pb.toConvexHull(); pbC.makeCCW();
            if (paC.verts.size() < 3 || pbC.verts.size() < 3) continue;
            Polygon pnfp = nfpConvexOrbital(paC, pbC);
            if (!pnfp.empty()) {
                for (const auto& v : pnfp.verts) allNFPVerts.push_back(v);
                partialNFPs.push_back(std::move(pnfp));
            }
        }
    }
    ...
    if (allNFPVerts.empty()) {
        ...
        Polygon Ah = A.toConvexHull(); Ah.makeCCW();
        Polygon Bh = B.toConvexHull(); Bh.makeCCW();
        Polygon nfp = nfpConvexOrbital(Ah, Bh);
        nfp.makeCCW();
        return {nfp};
    }

    // ── Polar-merge: строим огибающую по максимальному радиусу ────────────────
    // Центроид всех вершин — опорная точка для полярных координат
    Point center{0, 0};
    for (const auto& v : allNFPVerts) { center.x += v.x; center.y += v.y; }
    center.x /= allNFPVerts.size();
    center.y /= allNFPVerts.size();

    // Разбиваем [0, 2π) на SECTORS секторов, берём максимальный радиус в каждом
    constexpr int SECTORS = 128;
    std::vector<double> maxR(SECTORS, 0.0);
    std::vector<Point>  maxPt(SECTORS, center);

    for (const auto& v : allNFPVerts) {
        Point d = v - center;
        double r = d.length();
        double a = std::atan2(d.y, d.x);
        if (a < 0) a += PI2;
        int sector = (int)(a / PI2 * SECTORS) % SECTORS;
        if (r > maxR[sector]) {
            maxR[sector] = r;
            maxPt[sector] = v;
        }
    ...
    // Собираем вершины polar-envelope из секторов с ненулевым радиусом
    std::vector<Point> envelopeVerts;
    envelopeVerts.reserve(SECTORS);
    for (int s = 0; s < SECTORS; ++s)
        if (maxR[s] > GEO_EPS)
            envelopeVerts.push_back(maxPt[s]);

    Polygon nfp;
    if (envelopeVerts.size() >= 3) {
        nfp = Polygon(envelopeVerts);
        nfp.removeDuplicates();
        nfp.makeCCW();
    }

    // Если polar-merge вырожден — fallback на convexHull (безопасно)
    if (nfp.empty() || nfp.verts.size() < 3) {
        nfp = Polygon::convexHull(allNFPVerts);
        nfp.makeCCW();
    }
